...
import { HostGetById, HostSetById } from '../../services/hosts';
...
const HostEditComponent = (props) => {

    const { hostId, host } = useParams()
    ...
    const handleSave = () => {
        ...
        HostSetById({ id: hostId, info: db.Info }).then(async (json) => {
            if (json.error) {
                console.log(json);
                setFetchError(true)
    ...
    React.useEffect(() => {
        if (hostId !== '') {
            HostGetById({ id: hostId }).then(async (json) => {
                if (json.error) {
                    setFetchError(true)
...
export default HostEditComponent;